...
namespace wiser {
    ...
    public:
        ...
        /** 获取分词 N 值 */
        std::int32_t getTokenLength() const { return token_len_; }

        /** 获取压缩方式 */
        CompressMethod getCompressMethod() const { return compress_method_; }

        /** 是否启用短语搜索 */
        bool isPhraseSearchEnabled() const { return enable_phrase_search_; }

        /** 获取缓冲区刷新阈值 */
        std::int32_t getBufferUpdateThreshold() const { return buffer_update_threshold_; }

        /** 获取已索引文档数 */
        Count getIndexedCount() const { return indexed_count_; }

        /** 获取本次运行的最大索引文档数（-1 表示不限制） */
        std::int32_t getMaxIndexCount() const { return max_index_count_; }

        /** 是否已达到本次运行的索引上限 */
        bool hasReachedIndexLimit() const { return (max_index_count_ >= 0) && (indexed_count_ >= max_index_count_); }

        // 配置设置器
        /** 设置分词 N 值 */
        void setTokenLength(std::int32_t len) { token_len_ = len; }

        /** 设置压缩方式 */
        void setCompressMethod(CompressMethod method) { compress_method_ = method; }

        /** 启用/禁用短语搜索 */
        void setPhraseSearchEnabled(bool enabled) { enable_phrase_search_ = enabled; }

        /** 设置缓冲区刷新阈值 */
        void setBufferUpdateThreshold(std::int32_t threshold) { buffer_update_threshold_ = threshold; }

        /** 设置本次运行的最大索引文档数（-1 表示不限制） */
        void setMaxIndexCount(std::int32_t max_count) { max_index_count_ = max_count; }
        ...
        /** 获取搜索引擎组件 */
        SearchEngine& getSearchEngine() { return search_engine_; }
        /** 获取分词器组件 */
        Tokenizer& getTokenizer() { return tokenizer_; }
        /** 获取维基加载器组件 */
        WikiLoader& getWikiLoader() { return wiki_loader_; }

        // 索引缓冲区管理
        /** 获取倒排索引缓冲区 */
        InvertedIndex& getIndexBuffer() { return index_buffer_; }

        /** 获取当前缓冲计数 */
        Count getBufferCount() const { return buffer_count_; }

        /** 递增缓冲计数 */
        void incrementBufferCount() { ++buffer_count_; }

        /** 递增已索引文档计数 */
        void incrementIndexedCount() { ++indexed_count_; }

...
bool JsonLoader::extractStringField(const std::string& json_obj, const std::string& key, std::string& out) {
    ...
    p += needle.size();
    // 跳过空白与冒号
    while (p < json_obj.size() && std::isspace(static_cast<unsigned char>(json_obj[p]))) ++p;
    if (p >= json_obj.size() || json_obj[p] != ':') return false;
    ++p;
    while (p < json_obj.size() && std::isspace(static_cast<unsigned char>(json_obj[p]))) ++p;
    if (p >= json_obj.size() || json_obj[p] != '"') return false;
    ++p; // 进入字符串
    ...
    std::uint64_t total_objs = 0;
    {
        ...
        for (; i < n; ++i) {
            ...
            if (in_str) {
                ...
                if (c == '"') in_str = true;
                else if (c == '{') { if (brace == 0) ++total_objs; ++brace; }

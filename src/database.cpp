...
namespace wiser {
    ...
    Database::Database(Database&& other) noexcept { moveFrom(std::move(other)); }
    ...
    bool Database::initialize(std::string_view db_path) {
        ...
        if (!createTables()) { close(); return false; }
        if (!prepareStatements()) { close(); return false; }
        return true;
    }
    ...
    DocId Database::getDocumentId(std::string_view title) {
        if (!get_document_id_stmt_) return 0;
        ...
        if (rc == SQLITE_ROW) { return static_cast<DocId>(sqlite3_column_int(get_document_id_stmt_, 0)); }
        return 0;
    }

    std::string Database::getDocumentTitle(DocId document_id) {
        if (!get_document_title_stmt_) return "";
    ...
    std::string Database::getDocumentBody(DocId document_id) {
        if (!get_document_body_stmt_) return "";
    ...
    Count Database::getDocumentCount() {
        if (!get_document_count_stmt_) return 0;
        sqlite3_reset(get_document_count_stmt_);
        int rc = sqlite3_step(get_document_count_stmt_);
        if (rc == SQLITE_ROW) { return static_cast<Count>(sqlite3_column_int(get_document_count_stmt_, 0)); }
        return 0;
    }

    std::optional<TokenInfo> Database::getTokenInfo(std::string_view token, bool insert) {
        if (!get_token_id_stmt_) return std::nullopt;
    ...
    std::optional<PostingsRecord> Database::getPostings(TokenId token_id) {
        if (!get_postings_stmt_) return std::nullopt;
    ...
    bool Database::updatePostings(TokenId token_id, Count docs_count, const std::vector<char>& postings) {
        if (!update_postings_stmt_) return false;
        ...
        if (postings.empty()) {
            ...
            sqlite3_bind_blob(update_postings_stmt_, 2, postings.data(), static_cast<int>(postings.size()), SQLITE_STATIC);
        }
    ...
    bool Database::setSetting(std::string_view key, std::string_view value) {
        if (!replace_settings_stmt_) return false;
    ...
    bool Database::prepareStatements() {
        const struct {
            ...
            {"INSERT OR IGNORE INTO tokens (token, docs_count, postings) VALUES (?, 0, ?);", &store_token_stmt_},
    ...
    std::vector<std::pair<std::string, std::string>> Database::getAllDocuments() {
        ...
        if (!list_documents_stmt_) return docs;
